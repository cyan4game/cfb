<template>
  <view class="page">
    ...
    <view class="page-body page-body-0" v-if="pageIndex === 0">
      <view class="page-body-0-phone">
        <text class="page-body-0-phone-text">
          手机号码 +{{ user.internationalCode }} {{ user.mobilePhone }}
        </text>
      </view>
      <u-button color="#505bde" text="发送验证码" @click="sendOldPhoneCode"></u-button>
      <view class="page-body-0-other" v-if="user.jyPassword || user.googleState === '1'">
        <text class="page-body-0-other-label">无法验证原手机号？</text>
    ...
    <view class="page-body page-body-2" v-else-if="pageIndex === 2">
      <view class="page-body-2-phone">
        <text class="page-body-2-phone-text">
          已发送至 +{{ user.areaCode }} {{ user.mobilePhone }}
        </text>
      </view>
      ...
      <view class="page-body-2-timer">
        ...
        <text :class="{'page-body-2-timer-resend': true, 'page-body-2-timer-resend-disabled': oldCodeTimerTip !== '重新发送'}" @click="sendOldPhoneCode">
          {{ oldCodeTimerTip }}
        </text>
      </view>
      <u-button color="#505bde" text="下一步" @click="onOldPhoneCheck" :disabled="oldCode.length !== 6"></u-button>
    </view>
    <view class="page-body page-body-3" v-else-if="pageIndex === 3">
      <u-form class="page-body-3-form" labelPosition="left" :model="newForm" ref="newForm" labelWidth="0" :rules="newRules">
        <u-form-item prop="phone" borderBottom>
          <u-text color="#505bde" size="28rpx" text="请输入新手机号"></u-text>
          <u-input v-model="newForm.phone" placeholder="请输入手机号" border="bottom" clearable>
            <view class="phone-area" slot="prefix" @click="showAreaCode = true">
              <text class="phone-area-text">{{ newForm.areaCode }}</text>
              <u-icon size="32rpx" name="arrow-down-fill"></u-icon>
            </view>
          </u-input>
        </u-form-item>
      </u-form>
      <u-button color="#505bde" text="下一步" @click="sendNewPhoneCode" :disabled="newPhoneNextDisabled"></u-button>
    </view>
    <view class="page-body page-body-2" v-else-if="pageIndex === 4">
      <view class="page-body-2-phone">
        <text class="page-body-2-phone-text">
          已发送至 {{ newForm.areaCode }} {{ newForm.phone }}
        </text>
      </view>
      ...
      <view class="page-body-2-timer">
        <text class="page-body-2-timer-label">没收到验证码？</text>
        <text :class="{'page-body-2-timer-resend': true, 'page-body-2-timer-resend-disabled': newCodeTimerTip !== '重新发送'}" @click="sendNewPhoneCode">
          {{ newCodeTimerTip }}
        </text>
      </view>
      <u-button color="#505bde" text="下一步" @click="onNewPhoneCheck" :disabled="newCode.length !== 6"></u-button>
    </view>
...
import {checkGoogle, checkTxPassword, sendSMS, updatePhone, verifySMS} from "@/api/api";
...
export default {
  ...
  data() {
    return {
      ...
      bind: undefined,
      /**
       * 验证方式: phone,payPassword,googleCode
       */
      checkMod: '',
      pageIndex: 0,
      oldCode: '',
  ...
  computed: {
    ...
    newPhoneNextDisabled() {
      return !phoneReg.test(this.newForm.phone)
    },
    newRules() {
      return {
        phone: [
  ...
  created() {
    this.bind = !!this.user.mobilePhone
    this.pageIndex = this.bind ? 0 : 3
  },
  methods: {
    handleAreaCodeConfirm(e) {
      this.newForm.areaCode = '+' + e.value[0].split('+')[1];
    ...
    sendOldPhoneCode() {
      if (!this.$refs.oldCodeTimer || this.$refs.oldCodeTimer.canGetCode) {
        sendSMS({
          internationalCode: '+' + this.user.areaCode,
    ...
    sendNewPhoneCode() {
      if (!this.$refs.newCodeTimer || this.$refs.newCodeTimer.canGetCode) {
        sendSMS({
          internationalCode: this.newForm.areaCode,
          mobilePhone: this.newForm.phone,
          type: 2
        }).then(res => {
          if (res.code.toString() === '0') {
            this.pageIndex = 4
    ...
    onBindNewPhone() {
      updatePhone({
        ...
        if (res.code.toString() === '0') {
          uni.navigateTo({
            url: '/pages/safety/success?type=phone'
          })
        }
      })
    },
